...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--strategy', choices=['no_shard', 'shard_grad_op', 'full_shard'], required=True)
...
def main(args: argparse.Namespace) -> None:  # noqa: C901, PLR0915, PLR0912
    ...
    if args.strategy == 'no_shard':
        ...
        for layer in model.gpt_neox.layers:
            fully_shard(layer, mesh=mesh, mp_policy=mp_policy, reshard_after_forward=reshard_after_forward, offload_policy=offload_policy)

        fully_shard( model, mesh=mesh, mp_policy=mp_policy, reshard_after_forward=reshard_after_forward, offload_policy=offload_policy)
